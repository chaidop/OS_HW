...
static int team13_kyber_init_sched(struct request_queue *q, struct elevator_type *e)
{
	struct kyber_queue_data *kqd;
...
static void team13_kyber_exit_sched(struct elevator_queue *e)
{
...
static int team13_kyber_init_hctx(struct blk_mq_hw_ctx *hctx, unsigned int hctx_idx)
{
...
static void team13_kyber_exit_hctx(struct blk_mq_hw_ctx *hctx, unsigned int hctx_idx)
{
...
static void team13_kyber_limit_depth(unsigned int op, struct blk_mq_alloc_data *data)
{
	/*
...
static bool team13_kyber_bio_merge(struct blk_mq_hw_ctx *hctx, struct bio *bio,
...
static void team13_kyber_prepare_request(struct request *rq, struct bio *bio)
{
	rq_set_domain_token(rq, -1);
}

static void team13_kyber_insert_requests(struct blk_mq_hw_ctx *hctx,
...
static void team13_kyber_finish_request(struct request *rq)
{
...
static void team13_kyber_completed_request(struct request *rq, u64 now)
{
	struct kyber_queue_data *kqd = rq->q->elevator->elevator_data;
	struct kyber_cpu_latency *cpu_latency;
	unsigned int sched_domain;
...
static struct request *team13_kyber_dispatch_request(struct blk_mq_hw_ctx *hctx)
{
	struct kyber_queue_data *kqd = hctx->queue->elevator->elevator_data;
	struct kyber_hctx_data *khd = hctx->sched_data;
	struct request *rq;
	...
	spin_lock(&khd->lock);
	printk("In team13_kyber_dispatch_request function\n");
	/*
...
static bool team13_kyber_has_work(struct blk_mq_hw_ctx *hctx)
{
	struct kyber_hctx_data *khd = hctx->sched_data;
	int i;
...
static struct elevator_type team13_kyber_sched = {
	.ops = {
		.init_sched = team13_kyber_init_sched,
		.exit_sched = team13_kyber_exit_sched,
		.init_hctx = team13_kyber_init_hctx,
		.exit_hctx = team13_kyber_exit_hctx,
		.limit_depth = team13_kyber_limit_depth,
		.bio_merge = team13_kyber_bio_merge,
		.prepare_request = team13_kyber_prepare_request,
		.insert_requests = team13_kyber_insert_requests,
		.finish_request = team13_kyber_finish_request,
		.requeue_request = team13_kyber_finish_request,
		.completed_request = team13_kyber_completed_request,
		.dispatch_request = team13_kyber_dispatch_request,
		.has_work = team13_kyber_has_work,
	},
#ifdef CONFIG_BLK_DEBUG_FS
	...
	.elevator_name = "team13",
	.elevator_owner = THIS_MODULE,
};

static int __init kyber_init(void)
{
	return elv_register(&team13_kyber_sched);
}

static void __exit kyber_exit(void)
{
	elv_unregister(&team13_kyber_sched);
}

module_init(kyber_init);
module_exit(kyber_exit);

MODULE_AUTHOR("Heidi Poulianou");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Team13 Kyber I/O scheduler");
